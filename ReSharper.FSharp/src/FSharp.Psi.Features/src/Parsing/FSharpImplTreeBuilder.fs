...
type internal FSharpImplTreeBuilder(file, lexer, decls, lifetime) =
    ...
    member x.ProcessPat(PatRange range as pat, isLocal) =
        ...
        let elementType =
            ...
            | SynPat.Named (pat, id, _, _, _) ->
                ...
                if IsActivePatternName id.idText then x.ProcessActivePatternId(id) else () // todo: active patterns
    ...
    member x.ProcessBinding(Binding(_,_,_,_,attrs,_,_,headPat,_, expr,_,_) as binding, isLocal: bool) =
        ...
        let elementType = if isLocal then ElementType.LOCAL_BINDING else ElementType.TOP_BINDING // todo
